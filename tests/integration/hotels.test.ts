...
describe("GET /hotels/rooms/:hotelId", () => {
  it("should respond with status 401 if no token is given", async () => {
    const response = await server.get("/hotels/rooms/1");
  ...
  it("should respond with status 401 if given token is not valid", async () => {
    ...
    const response = await server.get("/hotels/rooms/1").set("Authorization", `Bearer ${token}`);
  ...
  describe("when token is valid", () => {
    it("should respond with status 404 for invalid hotel id", async () => {
      ...
      const response = await server.get("/hotels/rooms/100").set("Authorization", `Bearer ${token}`);

      expect(response.status).toEqual(httpStatus.NOT_FOUND);
    });

    it("should respond with status 200 and rooms with no bookings", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
      ...
      const response = await server.get(`/hotels/rooms/${createdHotel.id}`).set("Authorization", `Bearer ${token}`);
